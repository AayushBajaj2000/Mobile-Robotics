...
def callback(data):
    global published, counter
    # Publish it only once -> one reading, maybe we should increase these
    transformArray = data.markers
    
    if len(transformArray) > 0 and len(published) == 0:
        counter += 1
        transform = transformArray[0]

        if transform.pose.pose.position.x < 0.7:
            transform = transformArray[0]
            ...
            movebase_client(odom_x, odom_y, True)
            published.append("1")

            time.sleep(6)
            
            print("Sending to the marker now")                    
            system("rosrun turtlebot3_automatic_parking_vision automatic_parking_vision")

            #movebase_client(x_pos, y_pos, False)
